...
def search():
    search_what = txt_url.get()
    url_head = 'https://www.biqubao.com/search.php?keyword='
    url = url_head + par.quote(search_what, encoding="UTF-8")
    nov_headers = {
        "User-Agent": random.choice(uapools)
    }
    try:
...
if __name__ == '__main__':
    ...
    lab3 = tkinter.Label(root, text='搜索小说或者作者：', fg='blue').grid(row=2, column=0)
    txt_url = tkinter.Entry()
    txt_url.grid(row=2, column=1)
    btn = tkinter.Button(root, text='search', command=search, state='normal')
